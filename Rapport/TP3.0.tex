...
MetaCiv est un logiciel puissant qui repose sur de bonnes bases. Par exemple, le système d'injection d'actions personnalisé sous forme de code java est très bien conçu et le moteur du simulateur est fiable. Cela encourage à l'enrichissement du projet à de nouvelles fonctionnalités pour l'avenir.
...
\subsection{Noyaux}
	...
	Les évolutions à apporter au noyau découlent des besoins évoqués ci-dessus, la pluspart des attentes des modélisateurs peuvent être satisfaites par l'instauration d'un système de messages entre agents, que ce soit pour le commerce ou l'éducation.
	Pour modéliser les différences entre groupes ou civilisation, nécessaires à l'émergence de certains comportements comme par exemple les conflits, nous avons envisagé d'avoir des attributs de groupe qui représenterai les codes et normes de ces groupes, par exemple les croyance religieuses, les idéaux politiques ou encore la morale. Ces attributs de groupes peuvent êtres fluctuants et aussi servir à représenter différents états, pensez à l'opinion publique, ou aux effets de foules comme des manifestations qui dégénèrent à cause d'un incident n'impliquant que quelques uns des participants.
...
IMPRESSION PROJET
	SATISFAIT ? PK ?
RESULTATS
VOULU CHANGER (ce qu'on changerai)
CE QUE CA A APPORTE
