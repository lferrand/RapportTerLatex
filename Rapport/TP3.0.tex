...
La gestion de projet a joué un rôle important dans le développement des nouvelles technologie au cours du 20$^{ieme}$ et 21 $^{ieme}$ siècle. Si important qu'au vu du très grand nombre d'échecs de projet que dans les années 1990, il fut décidé de rechercher des méthodes différentes aux méthodes traditionnelles, comme la méthode dite de "Waterfall", pour gérer des projets informatiques. C'est la naissance de la méthode Agile et de son manifeste qui décrit les douze principes et quatre valeurs a respecter.\\
