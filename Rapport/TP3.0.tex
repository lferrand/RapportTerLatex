...
Une fois encore, on constate que le modèle est cohérent. Il permet rapidement d'observer des comportement de groupe mais sa vrai particularité est l'observation de l'adaptabilité de l'agent a son environnement. En effet le renforcement de cogniton par l'expérimentation des agents d'une façon un peu similaire a ce qu'on peut retrouver dans les colonies de fourmis permet d'aborder l'émergence d'une façon différentes. En effet chaque individu étudie lui même son environnement et c'est le regroupement de l'esprit de chaque individualité qui va créé la transition de civilisation et va permettre a la communauté de survivre plus longtemps. Cette modélisation a un niveau de dépendance a son environnement très élevé qui laisse beaucoup de place au hasard. En effet, on constate que puisque les agents font leur choix en fonction de leur environnement, la même civilisation mit dans deux contexte différent auront deux profil d'évolution totalement différente. Par conséquent, ce genre de modélisation est pratique pour "Observer" l'impact que peut avoir un environnement sur le fonctionnement d'un système multi agent, et dans ce cas la d'une civilisation humaine.
...
La gestion de projet a joue un rôle important dans le développement des nouvelles technologie au cours du 20eme et 21 eme. Tellement important que au vu du trop grand nombres d'échec et de non aboutissement de projet dans les années 1990 il fut décider de rechercher des méthodes différentes des méthodes traditionnel, comme le méthode dites de "waterfall", pour gérer des projet informatique. La naissance de la méthode Agile et son Manifeste qui décrit 12 principes et 4 valeur a respecter.\\

Nous Commencerons par donner une introduction a ce qu'est la méthode Agile pour ensuite décrire le déroulement du projet et comment nous avons fait pour respecter le plus possible les bonnes pratique demander. Finalement nous expliquerons les outils que nous avons utiliser pour faciliter la gestion de ce projet et donnerons nos conclusion quand a la réussite ou non de notre gestion.

\subsection{La méthode Agile}

La méthode agile repose sur des cycles de développement itératifs et adaptatifs en fonction des besoins évolutifs du client. Elle permet notamment d'impliquer l'ensemble des membres du groupe ainsi que les responsables du sujet dans le développement du projet.
Cet méthode permette généralement de mieux répondre aux attentes du client en un temps limité (en partie grâce à l'implication de ce dernier) tout en faisant monter le groupe en compétences. Cet méthode constitue donc un gain en productivité pour les deux côtés, ainsi qu'une flexibilité accrue.\\

Pour ce projet nous avons opté pour un sprint entre chaque réunion de groupe avec les tuteurs. MétaCiv étant déjà existant, dès les premières réunions nous avons pu présenter des modélisations ainsi que pointer du doigts certains problèmes, pour s'organiser pour les résoudre et également redéfinir les objectifs soulevés par sujet.

\subsection{Déroulement du Projet}

Ce projet avait de part ça nature quelque chose de spécial. En effet il s'agissait à la base d'un sujet de modélisation de societe humaine a travers l'utilisation de systemes multi agent, tout particulierement l'utilisation du logiciel MetaCiv. A cela venait s'ajouter l'obligation de devoir collaborer avec d'autres équipe travaillant aussi sur l'évolution de Metaciv.\\

Au vu la complexité que peut devenir une gestion, certe petites, de plusieur equipe. Avec la participation de Monsieur Ferber nous avons decider d'adopter l'utilisation de la méthode Agile. Cette méthode a était très efficace et nous a permis grâce a des sprints de une à deux semaines de nous permettre de montrer les avancer du projet et discuter des besoins, tout en redéfinissant les objectifs par rapport aux problèmes rencontrés. Lors de nos réunions hebdomadaires ou bi-mensuel , le groupe de modélisation présentait son avancée et les problèmes rencontrés et avec les conseils de Mr Ferber et Mr Stratulat , nous mettions au point des solutions qui permettent de pousser la modélisation plus loin.\\

Malgré cela, nos premières expériences de modélisation, qui se soldaient par des modèles corrompus et donc des heures de travail perdus , nous ont vite amené à la décision d'utiliser une partie de nos ressource à rendre le programme plus stable. Car dans un premier temps, les quatre membres devaient participer au développement des modèles de civilisations, mais ensuite le projet a évolué a cause des contraintes logiciel. François Suro s'est donc occupé a temps plein de la maintenance de MetaCiv ainsi que l'ajout de nouvelles fonctionnalité nécessaire a la Modélisation. Pendant ce temps le reste des membres s'occuper de l'objectif premier de notre projet, la modélisation. En parralele, Lionel Ferrand s'occupait aussi de toute la partie coordination et gestion de notre Logiciel et platforme de versionning GitHub.\\

Une fois cette étape atteinte , et le risque de perdre le travail effectué écarté , nous avons conservé cette séparation des tâches pour continuer la correction des nombreuse instabilité de Metaciv et le besoin croissant d'evolution dans le logiciel. Il nous fallait donc un outils pour facilement faire le reler entre les equipes et avoir un suivis efficasse des taches a accomplir. Nous avons decider d'utiliser Trello, un logiciel de Post-It en ligne que nous expliqueron plus en detail plus tard.\\

Les ambitions du projets ont donc été revu à la baisse tout en maximisant le travail qui était faisable avec les moyen et le temps qu'on possédait.

En somme , le développement à été conduit par les besoins de l'équipe de modélisation.
De plus, les avancées du logiciel que nous réalisions servaient également au deuxième groupe. Mais alors que la partie développement avait prit une place plus importante que prévu nous avons aussi pensait a la pérennité du projet en fournissant un code plus clair , en séparant bien chaque chose et évitant les fonctions de plusieurs centaines de ligne sans nécessité, maximisant les bonnes pratique de approche plus objet et en utilisant des design patterns.

\subsection{Trello}

Pour nous assister dans l'organisation des tâches à faire, nous avons utilisé un outil de gestion de projet en ligne, Trello. Ce programme est basé sur une organisation des projets en planches listant des cartes (tâches à faire, sorte de Post It virtuel). Les cartes sont assignables à des utilisateurs et sont mobiles d'une planche à l'autre, traduisant leur avancement. Ainsi nous avons pu organiser le travail à faire en catégorie, pour savoir ce qu'il nous restait à faire et indiquer les fonctionnalité qu?on souhaitait voir, en pouvant cibler le destinataire du message tout étant visible par tous.

\subsection{GitHub}

Pour le développement des avancés  de MetaCiv, nous avons utilisé GitHub. GitHub est un service web pour les projets de développement de logiciels qui utilisent le système de contrôle de révision Git. Un système de contrôle de révision est la gestion des modifications apportées aux documents, programmes informatiques et autres collections d'informations. Les changements sont généralement identifiées par un code numérique ou alphabétique, appelé le "numéro de révision", "niveau de révision", ou simplement «révision». Par exemple, un ensemble initial de fichiers est "révision 1". Lorsque le premier changement est effectué, l'ensemble résultant est "révision 2", et ainsi de suite. Chaque révision est associée à un horodatage et à la personne qui fait le changement. Les révisions peuvent être comparés, restaurés, et avec certains types de fichiers, fusionnées. Il est une étape obligée pour les équipes de développement aujourd'hui.
