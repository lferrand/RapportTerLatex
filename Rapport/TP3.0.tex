...
Ce projet nous avait vraiment plu dès le départ puisque nous avons tous été très intéressé par la programmation orienté objet depuis notre première approche avec WarBot. 
Nos impressions négatives sur le projet:
\begin{itemize}
\item Les planages successifs sans aucun rapport d'erreur rendaient la modélisation difficile.
\item Temps d'adaption un peu lent
\item La lenteur inexpliqué sur certains oridnateur
\end{itemize}

Nos impressions positives :
\begin{itemize}
\item Les actions étaient très facile à implémenter ce qui était un plus pour la modélisation.
\item 
\end{itemize}
...
EVOLUTION PERSONNELLES ET DU PROJET
	FUTUR ?
