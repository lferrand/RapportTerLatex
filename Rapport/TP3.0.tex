...
Dans ce modèle, on réalise une émergence de civilisation à travers une société dépendante de son environnement. L'objectif est de montrer qu'une société peut à travers l'observation et l'adaptation à son environnement survivre, s'adapter et évoluer. Pour ce faire, on a décidé de simuler une société nomade sans chefs et très dépendante de son environnement. La spécialisation des agents sera basé sur le renforcement de leurs cognitons à travers l'expérience de la réalisation ou de l'échec de leurs plans.

\subsubsection{Introduction}

Ce document expliquera le fonctionnement d'une modélisation qui a pour but de mettre en valeur l'émergence d'une civilisation a travers une société nomade jusqu'à l'étape d'agriculture. Cette implémentation se base sur MetaCiv, framework de modélisation de sociétés humaines utilisant la technique des SMA. Toutes les constantes mentionnées dans ce documents sont données dans la section "\hyperref[constantes]{Constantes}".

L'objectif de cette modélisation est d'observer comment une société sans chef a proprement nommer va conduire sa population a survivre dans un environnement avec différente façon d'obtenir des ressource et de faire naitre le besoin de faire de l'agriculture une fois que la population devient trop élever pour être soutenu par un système nomade.
...
\subsubsection{Les actions par défaut}
...
schema des birthplan autoplan chasse\\
...
schema Peche / agriculture / Déménager\\
...
\subsubsection{Constantes}
	...
	- permettre la mise à jour de la valeur des constantes pendant l'exécution de la simulation
	- permettre de choisir entre l'utilisation des constantes et l'utilisation d'une valeur unique
	- assurer la rétro-compatibilité des actions tout en donnant la possibilité de les adapter avec un minimum de changements	
	
	Nous avons décidé d'implanter le design pattern Observateur.
	Pour ce faire, nous avons crée une classe MCConstant qui hérite de Observable et contient la valeur numérique de la constante, cette classe est manipulée directement par l'interface de gestion des constantes.
...
\caption{Méthode d'initialisation, on peut voir l'initialisation avec la valeur numérique , puis si le lien à une constante à été spécifié , la valeur est changé pour celle de la constante et le lien Observable-Observer est mis en place}
\end{figure} 

	pour ne pas avoir à dupliquer les actions , celles qui utilisent les constantes et celles qui utilisent les valeurs directes , nous proposons systématiquement l'utilisation d'une constante dans l'interface pour tout les paramètres de type numériques. Si l'action n'a pas été mise à niveau , la constante sera simplement ignorée et la valeur directe sera prise en compte ce qui assure la rétro-compatibilité.
	
\begin{figure}[H]
\center
\includegraphics[width=12cm]{img/DEV-Const-InterfaceAction.png}
\caption{interface de paramètrage d'une action, on peut choisir l'utilisation de la valeur directe ou d'une constante à l'aide du bouton radio, la combo box "linked constant" propose toutes les constantes du schéma cognitif en cours d'édition}
\end{figure} 
...
\end{itemize}	
	...
	avant de lancer d'écraser les fichiers avec la nouvelle sauvegarde , on effectue une copie des fichiers d'origine dans un dossier backup situé a coté du dossier du modèle.
...
\subsubsection{Defines}
	besoins : - centraliser les valeurs constantes utilisés dans le code du programme afin de faciliter les modifications éventuelles
	
	Dans le but de favoriser les développements futurs, nous avons crée des classes Define contenant des variables statiques qui sont apellés à différents endroits du programme.
	
\begin{figure}[H]
\center
\includegraphics[width=10cm]{img/DEV-DefinePath.png}
...
\begin{itemize}
 \item Permettre la suppression d'éléments du schéma cognitif
 \item Ajout d'un bouton qui permet une fermeture propre du programme (sans laisser de zombies)
...
introduire un plan qui s'exécuterait au premiers instants de la simulation , pour chaque agent. une des applications serait de modifier l'âge de départ des agents de manière aléatoire pour représenter le fait que la population de départ a déjà vécu un certain temps et est composée d'individus d'ages différents.

\subsubsection{Hiérarchie de cognitons}
création d'un type de plan spécial qui conduit à l'évaluation d'un nouveau groupe de cognitons. Les applications d'un tel système sont multiples
	
\begin{figure}[H]
\center
\includegraphics[width=12cm]{img/DEV-ID-strategies.jpg}
\caption{appliqué à une répartition en sous rôles : si l'action chasser est décidé, l'agent va ensuite décider si il sera archer ou rabatteur}
\end{figure} 
	
\begin{figure}[H]
\center
\includegraphics[width=12cm]{img/DEV-ID-reflexes.jpg}
\caption{utilisé comme système de subsomption : les besoins essentiels sont évalués à la première étape comme la faim ou le sommeil, si ces besoins sont satisfaits , le cogniton satisfait prends le dessus et déclenche le mode 2 qui évalue des actions plus complexes}
\end{figure} 
	
\subsubsection{Objets uniques / stackables}
Séparer les Objets en deux catégories : les objets uniques qui possèdent des attributs comme l'usure de l'objet et les objets dits "stackables" (litt. "empilable") qui resteront toujours identiques , par exemple les ressources alimentaires.
...
\begin{itemize}
\item gestion des plans : au chargement , certains plans sont ajoutés deux fois (mais la simulation fonctionne quand même)
	\begin{itemize}    
    \item un test toujours vrai dans le chainage des plans cause l'ajout en double
    \item nettoyage du code , le problème apparait clairement et est résolu (note : amélioration sensible des performances)

\end{itemize}

\item gestion des triggers : la modification des triggers ne semble pas avoir d'effet
	\begin{itemize}    
    \item présence de deux listes représentant les triggers : une est utilisé par les agents , l'autre par l'interface.
    \item regroupement en une seule liste , modification de la liste au lieu de détruire et reconstruire (permet la modification des valeurs pendant l'exécution)

\end{itemize}

\item esprit de l'agent : certain cognitons sont présent en double , ce qui fausse le calcul de l'action à effectuer
     \begin{itemize}   
     \item les Starting cognitons sont ajoutés en plus de la liste de tous les cognitons qui les contient déjà 
     \item sécurisation de la liste des cognitons d'un agent à travers un accesseur , si le cogniton est déjà présent , l'ajout est ignoré

\end{itemize}

\item Sauvegarde : la sauvegarde sous windows corrompt certain modèles (pas sur mac)
	\begin{itemize}    
    \item pas de vérification des caractères interdits dans le nom des éléments qui sont utilisés pour nommer le fichier correspondant
	\item utilisation de URLencoder sur toutes les chaines servant comme nom de fichier 
